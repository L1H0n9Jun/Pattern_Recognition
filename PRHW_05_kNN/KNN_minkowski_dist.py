...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=True,
        help="""
        The dist type for kNN, which is d-norm.
        """
    )
    parser.add_argument(
        '-n', '--num_train',
...
def comp_dist(test_sample, train_sample, d):
    ''' 计算闵氏距离，即不同范数 '''
    ...
    for i in range(test_num):
        ...
        dist[i, :] = np.power(
            np.sum(np.absolute(test_img - train_sample)**d, axis=1), d)
        #dist[i, :] = np.sqrt(np.sum((test_img - train_sample)**2, axis=1))
    return dist
...
def pred_label(test_sample, train_sample, train_label, d=2, k=2):
    ''' 根据定义的距离和k值对测试集进行分类 '''
    dist = comp_dist(test_sample, train_sample, d)
